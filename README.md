...
An end-to-end credit-card fraud detection system covering the engineering progression from a CPU baseline to GPU-accelerated inference: **Python/CPU → PyTorch GPU → custom CUDA kernel → C++ inference → TensorRT → containerized API.**

Not a production system: single-machine, single-GPU, no live transaction data, no SLA. Every number in this README is generated by a script in this repo; none are hand-typed.
...
See [Environment Notes](#environment-notes) below for why WSL2 was needed for the GPU phases and how the driver/toolchain compatibility issues were resolved.
...
`src/models/model.py` — a deliberately small MLP (29 → 64 → 32 → 1, 4,033 parameters), kept small so every downstream port (CUDA kernel, C++ rewrite, TensorRT export) stays tractable to implement and verify.

`src/models/train.py` — trains with `BCEWithLogitsLoss` and a `pos_weight` computed from the train-split class ratio (582:1) to counter the imbalance, rather than resampling. Accuracy is not used as a metric — see the module docstring for why (with 0.17% fraud, "always predict legitimate" scores 99.83% accuracy while catching zero fraud).
...
**Results (seed=42, 20 epochs, CPU, run 2026-08-20):**
...
Test set: 41,350 transactions, 71 fraud. Full per-epoch history and both threshold evaluations are saved to `models/checkpoints/training_results.json` on every training run.
...
Raw data: `benchmarks/results/cpu_results.json`. The takeaway: with a 4,033-parameter model, per-call Python/dispatch overhead dominates at small batches — latency barely grows from batch 1 to 1024 (0.07ms → 0.16ms), so throughput scales almost linearly with batch size. This raises the question Phase 5 investigates: is there a bottleneck worth optimizing with CUDA here, or is a model this small already dispatch-bound rather than compute-bound?
...
`src/inference/pytorch_gpu.py` — the GPU counterpart to `cpu_inference.py`, same `predict()` interface, model moved to `cuda`, inference wrapped in `torch.inference_mode()`-equivalent (`@torch.no_grad()`), benchmark timing wrapped with `torch.cuda.synchronize()` so async kernel launches are measured rather than just their dispatch.

**Environment resolution (see [Environment notes](#environment-notes) for details):** the installed driver caps CUDA at 11.4, incompatible with the CUDA 12.x builds current PyTorch/Python 3.13 require. Rather than block on a driver update, GPU work runs in a second venv, `.venv-gpu` (Python 3.10, installed at user scope via `winget --scope user` — no admin needed), with `torch==1.12.1+cu113`, whose CUDA 11.3 runtime the current driver supports. Model checkpoints are saved as plain `state_dict` tensors, so the same `fraud_mlp.pt` trained under torch 2.13/CPU loads and runs correctly under torch 1.12/CUDA — verified by `tests/test_gpu_inference.py::test_gpu_and_cpu_agree_numerically`, which checks CPU and GPU produce numerically equivalent probabilities on identical weights/inputs (`atol=1e-5`).
...
**Results** (same batch sizes, warm-up, and iteration counts as the CPU baseline; run 2026-08-20):
...
**The GPU is slower than the CPU at every batch size tested.** At 4,033 parameters, the model's compute is trivial — kernel launch overhead and host↔device transfer dominate the GPU's wall-clock time, and that fixed overhead isn't amortized because there isn't enough work per call to hide it behind. This is a common result for small tabular models, and is why Phase 5 investigates the pipeline stage-by-stage before writing a custom CUDA kernel rather than assuming the GPU path is faster.
...
Before writing a custom CUDA kernel, `benchmarks/profile_pipeline.py` measures where the GPU pipeline spends time — host tensor prep, H2D transfer, GPU compute, D2H transfer, postprocessing — each stage isolated with `torch.cuda.synchronize()` boundaries so async GPU work can't leak into the wrong measurement.
...
**Conclusion driving Phase 6:** fusing the 3-layer MLP forward pass into a single custom CUDA kernel replaces ~6 kernel launches with 1, directly targeting the ~70% of pipeline time measured here as launch-overhead-bound. Whether the fused kernel wins is what Phase 6 measures.
...
**Results** (same harness/methodology as Phases 3-4):
...
The kernel beats PyTorch's own GPU dispatch at every batch size (confirming Phase 5's diagnosis), but only beats the CPU baseline at batch≥512 — a crossover, not a uniform GPU win. See docs/custom_cuda_kernel.md for the full reasoning.
...
`cpp/inference.cpp`'s `FraudPredictor` has no libtorch/PyTorch dependency in the call path. It reads the exported raw weight binaries, uploads them to the GPU once at construction, and reuses persistent input/output device buffers across calls (growing only on demand) — unlike Phase 6's PyTorch extension, which re-wraps a `torch::Tensor` on every call. It calls `cuda/fraud_kernel.cu`'s kernel directly. Full design + validation writeup: [docs/cpp_inference.md](docs/cpp_inference.md).

**Correctness:** `cpp/test_predictor.cpp` — input validation (wrong feature count → `std::invalid_argument`), missing-weights handling (→ `std::runtime_error`), and **10/10 held-out labeled transactions classified correctly**. All pass.

**Results** (same batch sizes/methodology as every other backend):
...
Comparable to, and at batch 32-512 modestly faster than, the PyTorch-extension kernel (consistent with shedding Python/PyTorch dispatch overhead) — but not uniformly faster (slower at batch=1024, most plausibly measurement noise since both paths call the identical kernel). Full table in docs/cpp_inference.md.
...
No NVIDIA account or browser login needed, though getting there took some troubleshooting. Plain `pip install tensorrt` installs fine but bundles CUDA 12.9 runtime regardless of version number, which fails against this driver (`CUDA initialization failure with error: 35`). The CUDA-11-suffixed `tensorrt-cu11==10.0.1` package resolves to CUDA-11-compatible libraries and works — verified by building and executing an engine, not just constructing a `Builder`. Full troubleshooting log (build isolation stripping `pip`, a cudnn8-vs-cudnn9 mismatch) and design writeup: [docs/tensorrt.md](docs/tensorrt.md).
...
**Correctness:** 10 tests, including **10/10 held-out labeled transactions classified correctly**. Numerical-agreement tolerance vs CPU is looser than Phase 6/7 (TensorRT reorders floating-point ops during kernel fusion — measured max diff ~7e-4, expected TensorRT behavior, not a bug).
...
**TensorRT is the slowest GPU path at every batch size** — slower than CPU, the custom kernel, and C++. A fully static-shape engine (no dynamic profile) performed statistically identically (0.183ms vs 0.185ms), ruling out shape flexibility as the cause. TensorRT's runtime overhead (shape validation, dispatch through Python/pycuda bindings, stream sync) isn't amortized by a model this small — TensorRT is built for production-scale models with millions+ of parameters, and a 4,033-parameter MLP isn't representative of that workload. Full reasoning in docs/tensorrt.md.
...
**Engine defaults to CPU**: every benchmark since Phase 3 measured CPU as the fastest backend at batch_size=1 (0.070ms vs 0.091-0.239ms for every GPU path), and a REST API's `/predict` serves one transaction per call. Other engines (`pytorch_gpu`, `custom_cuda`, `tensorrt`) are selectable via the `FRAUD_API_ENGINE` env var, with automatic fallback to CPU if the requested engine isn't available in the current environment (e.g., requesting `tensorrt` inside a plain CPU container).

**Verified with a running server**, not just TestClient — hit over HTTP:

```
GET /health  -> {"status": "ok", "engine": "cpu", "model_loaded": true, "decision_threshold": 0.9992}

POST /predict  (fraud transaction, test-split index 31474)
  -> {"fraud_probability": 1.0, "is_fraud": true, "engine": "cpu", "latency_ms": 1.66}

POST /predict  (legitimate transaction, test-split index 40337)
  -> {"fraud_probability": 1.04e-07, "is_fraud": false, "engine": "cpu", "latency_ms": 0.42}

POST /predict  (3 features instead of 29) -> 422
```

**Tests:** `tests/test_api.py` — 7 tests: health check, valid prediction, wrong feature count, missing field, non-numeric input, malformed JSON, and **all 10 sample transactions classified correctly through the live API path**. All pass.
...
Docker Desktop's Windows installer is admin-gated. Installing Docker Engine directly inside WSL2 via `apt install docker.io` avoids it — no GUI installer, no reboot. Full story: [docs/docker.md](docs/docker.md).

Containerizes the REST API with the CPU engine (consistent with Phase 9's default). Lean image — only what the API imports at runtime (`torch` CPU, `fastapi`, `uvicorn`), not the full training/preprocessing dependency set.

**Limitation:** the GPU/CUDA/TensorRT backends are not containerized — GPU passthrough into a WSL2-hosted Docker Engine needs its own setup (NVIDIA Container Toolkit) that wasn't configured here. `FRAUD_API_ENGINE` supports selecting them, but that would need the additional setup to work in this container.

**Verified** — built the image, ran the container, hit it over HTTP:
...
Fraud transaction → `probability: 1.0`, legitimate transaction → `probability: 1.04e-07` — identical to Phase 9's non-containerized results. `docker-compose up -d` verified separately with the same outcome.
...
`frontend/index.html` — a single static page, served by the API itself (`src/api/server.py` mounts it at `/`). Every value shown is fetched from the running API:

- "Load Legitimate/Fraud Example" buttons call `GET /samples`, which reads the held-out test-split transactions (`data/samples/sample_transactions.csv`, same fixtures used by the C++/TensorRT/API tests since Phase 7) — not fixture data embedded in the page's JS.
- "Run Inference" POSTs the current 29 feature values to `POST /predict` and renders exactly what the API returns.
- Latency shown is the server's own measured `latency_ms`, not a client-side timer.
- If the API is unreachable or returns an error, the page shows an error message rather than a fallback prediction.

**Verified end to end**: exercised the same sequence the page's JS performs — `GET /samples` → pick a fraud row (test-split index 31474) → `POST /predict` with its features → `{"fraud_probability": 1.0, "is_fraud": true, "engine": "cpu", "latency_ms": 3.70}`. Also covered by `tests/test_api.py`'s `test_full_frontend_flow_sample_to_prediction` (11 tests total in that file, all pass).
...
**`scripts/check_environment.py`** — reports what's available (GPU, PyTorch+CUDA, TensorRT, nvcc, C++ compiler, CMake, Docker) via subprocess calls and imports, not hardcoded values. Reports different output on each of this project's three environments — confirmed by running it on both the plain Windows CPU venv and the WSL2 full-stack venv side by side.

**`scripts/demo.py` / `scripts/demo.sh`** — one-command terminal demo built around the backend-vs-backend comparison, not a single API call. Loads the trained model and labeled sample transactions, warms up each backend once (the first GPU call's one-time CUDA/cuDNN cost — measured at ~4.6 seconds on this machine — would otherwise dominate the first result), then for each of 4 transactions prints every available backend ranked fastest-to-slowest with a text bar chart, plus an aggregate mean-latency summary at the end. On WSL2, all 4 backends (CPU, PyTorch GPU, custom CUDA kernel, TensorRT) run side by side — 16/16 correct predictions in the verified run. On the plain Windows venv it shows CPU only. The script also notes that its single-call live numbers are noisier than the formal 200-iteration benchmark and can show a different backend order for close pairs — the benchmark tables above are the basis for the project's conclusions.

**`scripts/verify.py` / `scripts/verify.sh`** — runs (not just imports) every major component and reports PASS/FAIL/SKIP: loads and runs the model, calls each inference backend with real data, hits the API's `/health` and `/predict`, checks for Docker, and runs the full pytest suite. SKIP means a dependency isn't installed in this environment, not a failure; FAIL means something that should have worked didn't. Verified run on the WSL2 full-stack environment: 13 passed, 0 skipped, 0 failed (68/68 tests in the full suite).

Building `verify.py` surfaced two bugs, both fixed:
...
2. `check_environment.py`'s TensorRT detection used `importlib.util.find_spec("tensorrt")`, which returns a truthy result even when the TensorRT package isn't installed, because this project's own `tensorrt/` directory gets picked up as a Python 3 namespace package on `sys.path` — the collision risk noted in Phase 8. Fixed by checking `tensorrt.__file__ is not None` after import, which distinguishes the real package from the namespace-package shadow.
...
`benchmarks/plot_results.py` reads the JSON results from every phase's benchmark script (Phases 3/4/6/7/8) and generates these directly. `benchmarks/generate_summary.py`'s table output was diffed against every number in this README as a correctness check (all matched).
...
The batch-size chart summarizes the crossover documented phase by phase above: CPU starts fastest at small batches, the custom CUDA kernel and C++ path overtake it once the batch is large enough to amortize GPU overhead, and PyTorch-eager-mode/TensorRT never catch up for a model this small.
...
**Phase 6 (custom CUDA kernel) is where native Windows compilation hit a wall.** `nvcc` 11.3 only supports MSVC 2017-2019 as a host compiler; this machine's installed MSVC is a much newer preview toolset whose own STL headers require CUDA 12.4+. Four attempts to install a compatible older MSVC toolset side-by-side (pinned-version winget install, `vs_installer modify` to add the legacy v141 component, a from-scratch bootstrapper install) all failed for concrete, verifiable reasons (~7GB downloaded across the attempts) — a limitation of this machine's specific VS/CUDA version combination.
...
- [x] Phase 7 — C++ inference (persistent device buffers, no PyTorch dependency; 10/10 transactions correct, see docs/cpp_inference.md)
- [x] Phase 8 — TensorRT (works via tensorrt-cu11, no manual/account action needed; the slowest GPU path for this model size, see docs/tensorrt.md)
- [x] Phase 9 — REST API (FastAPI, defaults to CPU per Phase 3-8 data, verified with a running server over HTTP)
- [x] Phase 10 — Docker (Docker Engine in WSL2, no manual action needed; build+run+compose verified, see docs/docker.md)
- [x] Phase 11 — full test suite (68 tests total; GPU/CUDA/TensorRT-specific tests skip cleanly when their dependency isn't in the current environment — see `scripts/verify.py`)
...
- [x] Phase 13 — profiling (stage-by-stage GPU pipeline profiling in Phase 5, `nvidia-smi`/CUDA-event measurements — see docs/bottleneck_analysis.md)
- [x] Phase 14–15 — final repo polish + documentation

## Limitations

Consolidated from the per-phase sections above:
...
- **GPU-enabled Docker image** — add NVIDIA Container Toolkit configuration and a second Dockerfile target for the CUDA backends, verified the same way as the CPU image (built, run, and hit over HTTP).
