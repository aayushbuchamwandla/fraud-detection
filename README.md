...
**`scripts/demo.py` / `scripts/demo.sh`** — one-command terminal demo, built around the backend-vs-backend comparison as the centerpiece, not a single API call. Loads the real trained model and real labeled sample transactions, warms up each backend once (so the first GPU call's one-time CUDA/cuDNN cost — measured at ~4.6 seconds on this machine — doesn't get shown as if it were the model's real latency), then for each of 4 real transactions prints every available backend **ranked fastest-to-slowest with a text bar chart**, plus an aggregate mean-latency summary across all transactions at the end. On WSL2, all 4 backends (CPU, PyTorch GPU, custom CUDA kernel, TensorRT) run side by side — **16/16 correct predictions** in the verified run. On the plain Windows venv it gracefully shows CPU only. The script also prints an honest caveat that its single-call live numbers are noisier than the formal 200-iteration benchmark and can show a different backend order for close pairs — the README's benchmark tables above are what the project's actual conclusions are based on.
