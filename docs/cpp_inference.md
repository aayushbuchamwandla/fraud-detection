...
Not a wrapper that calls Python — `cpp/inference.cpp`'s `FraudPredictor` is a real inference engine: it reads the exported raw weight binaries directly, owns persistent GPU device buffers (weights uploaded once at construction; input/output buffers reused across calls and grown only on demand), and calls `cuda/fraud_kernel.cu`'s kernel directly with no PyTorch/libtorch dependency anywhere in the call path.
...
- `predict_batch_flat(ptr, batch_size)` — the actual entry point both convenience methods funnel through, and what the benchmark harness calls directly to avoid `std::vector<std::vector<float>>` copy overhead when timing.
...
**Why persistent buffers matter here specifically:** the PyTorch extension path (Phase 6) re-wraps a `torch::Tensor` — with ATen's own allocator bookkeeping — on every `predict()` call. `FraudPredictor` allocates its input/output device buffers once and reuses them, only growing (never shrinking) when a larger batch shows up. This is a genuine architectural difference from Phase 6, not the same thing wearing a C++ syntax.
...
`cpp/test_predictor.cpp`, run against the real trained weights and the real labeled sample transactions (`data/samples/sample_transactions.csv`, pulled from the held-out test split — see `scripts/export_sample_transactions.py`):

- Input validation: wrong feature count → `std::invalid_argument` (checked, not assumed)
- Missing weights directory → `std::runtime_error` (checked, not assumed)
- All 10 real sample transactions produce probabilities in `[0, 1]`, **10/10 classified correctly** against true labels
...
The C++ path is comparable to, and at batch sizes 32-512 modestly faster than, the PyTorch-extension version of the identical kernel — consistent with removing Python/PyTorch dispatch overhead (`torch::Tensor` construction, ATen op dispatch) from the call path. **It is not uniformly faster**: at batch=1024 the C++ path measured slower than the PyTorch extension (0.129ms vs 0.118ms). Given both call the exact same `launch_fraud_mlp_forward` kernel, this crossover is most plausibly measurement noise (WSL2/system jitter) rather than a real architectural effect at that batch size — reported as measured rather than smoothed over or re-run until it looked better. The crossover against the CPU baseline (C++ wins at batch≥512, same as Phase 6's kernel) is consistent with Phase 5/6's conclusion: this model is small enough that GPU paths only pay off once the batch is large enough to amortize fixed per-call overhead.
