...
Docker Desktop's Windows installer is admin-gated. This is avoided by installing Docker Engine directly inside WSL2 via `apt`, instead of Docker Desktop for Windows:
...
This works because WSL2 runs a real Linux kernel — `docker.io`'s daemon runs the same way it would on any Ubuntu server, no Hyper-V/WSL2-backend GUI flow, no reboot. The passwordless `sudo` set up in Phase 6 for the CUDA toolchain covers this too.
...
The first build/run of the image (before the web frontend was added) worked correctly. After adding the frontend and its `GET /samples` endpoint, testing the container showed `/samples` returning `[]` — the Dockerfile copied `src/` and `frontend/` but not `data/samples/`, so the sample-transaction CSV wasn't in the image. Fixed by adding `COPY data/samples/ data/samples/`, rebuilt, and reverified the endpoint returns the 10 labeled transactions.
...
The CUDA/TensorRT/custom-kernel/C++ backends are **not** containerized: GPU passthrough into a WSL2-hosted Docker Engine is a separate configuration surface (NVIDIA Container Toolkit, `--gpus` runtime flags) not set up here. `FRAUD_API_ENGINE` supports selecting them, but doing so inside this container would fail without that additional setup.
...
# Fraud transaction (test-split index 31474):
{"fraud_probability": 1.0, "is_fraud": true, "engine": "cpu", "latency_ms": 1.62}

# Legitimate transaction (test-split index 40337):
